...
namespace gcem
{
    ...
    namespace internal
    {
        ...
        template <typename T>
        constexpr T
        sqrt_simplify(const T x, const T m_val) noexcept
        {
            return (x > T(1e+08) ? sqrt_simplify(x / T(1e+08), T(1e+04) * m_val) : x > T(1e+06) ? sqrt_simplify(x / T(1e+06), T(1e+03) * m_val)
